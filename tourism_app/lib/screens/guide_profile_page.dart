...
class _GuideProfileScreenState extends State<GuideProfileScreen> {
  ...
  final List<String> _languages = ["English", "Hindi", "Tamil", "Telugu", "French",
    "German", "Japanese"];
  final List<String> _countries = [ "India", "United States", "United Kingdom", "Canada", "Germany",
  ...
  Future<void> updateGuideData() async {
    ...
    try {
      ...
      print("Error updating guide data: $e");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFFFDEDD2),
        elevation: 0,
        leading: const BackButton(color: Color(0xFF770A1D)),
        title: const Text(
          'Profile',
          ...
          child: Column(
            children: [
              GestureDetector(
                onTap: () {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("You can't edit the email")),
                  );
                },
                child: AbsorbPointer(
                  child: buildInputField(Icons.email, _emailController, "Email"),
                ),
              ),
              const SizedBox(height: 15),
              buildInputField(Icons.account_circle, _usernameController, "Username"),
              const SizedBox(height: 15),
              buildDropdown("Language", _languages, _selectedLanguage, (value) {
                setState(() => _selectedLanguage = value);
              }),
              const SizedBox(height: 15),
              buildDropdown("Country", _countries, _selectedCountry, (value) {
                setState(() => _selectedCountry = value);
              }),
              const SizedBox(height: 30),
              ElevatedButton(
                onPressed: updateGuideData,
  ...
  Widget buildInputField(IconData icon, TextEditingController controller, String label) {
